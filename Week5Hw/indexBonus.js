let toDo = [
  ...
  toDo.forEach((user) => {
    if (user.userId == 3 || user.userId == 7 || user.userId == 8) {
        allToDos.push(user);
    }
}
   );
   ...
   let finalVar = allToDos.map((val, i, allToDos) => {

    return val.userId = {
        wow : 7
        // totalTasks: val.title,
        // totalIncomplete: val.body,
        // incompleteTasks: val.body,
    };
});
